...
ZCP_MinWaitTime = 300; // seconds to wait to spawn a new capturepoint when 1 was capped.
ZCP_BaseCleanupDelay = 300; // seconds to wait to delete a captured base.
ZCP_MaxMissions = 3; // Ammount of cap points at the same time.
ZCP_Minimum_Online_Players = 2; // Ammount of players to be online before it allows to spawn capture points. !!! O = always
...
ZCP_ServerMaxPlayers = 50; // Only needed to recalcuate the reward.
ZCP_MinKryptoReward = 200; // The crypto reward can never be recalculated lower then this.

ZCP_VehicleReward = ["B_Truck_01_box_EPOCH","B_Truck_01_box_EPOCH","B_Truck_01_box_EPOCH"]; // Vehicle classes for vehicle reward

ZCP_WeaponReward = [["LMG_Zafir_F",1],["srifle_EBR_DMS_F",1],["Machinegun_arifle_MX_SW_F",1],["arifle_MX_SW_Black_F",1],["1911_pistol_epoch",1],["arifle_TRG21_F",1]]; // Classname + amount in an array in the reward array for weapons
ZCP_ItemWeaponReward = [["optic_Arco",1]]; // Ammo is assigned automaticly.
...
ZCP_CapPoints = [ // [name,[x,y,z],reward, varname,index]
	...
	["Cap Point Bravo",[13431.3,19417.8,0],"WeaponBox","beta",1] ,
	["Cap Point Charlie",[9895.33,8804.58,0.0010643],"BuildBox","charlie",2] ,
	["Cap Point Delta",[26650.3,24181.5,0],"Vehicle","delta",3] 
];
